<div class="{{ $block->classes }} block-donation-types" style="{{ $block->inlineStyle }}">
  ...
  <ul class="block-donation-types__list">
    @foreach($items as $key => $item)
      <li class="block-donation-types__item item on-appear" style="background-image: url({{ $item->background }}); animation-delay: {{ $key }}s">
